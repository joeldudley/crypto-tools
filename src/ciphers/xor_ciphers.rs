...
mod tests {
    ...
    fn can_encrypt_with_repeating_key_xor() {
        ...
        let ciphertext = repeating_key_xor_cipher(plaintext.as_bytes(), key.as_bytes());
        let expected_ciphertext_bytes = hex::decode(expected_ciphertext_hex)
            .expect("could not convert hex to bytes");
        assert_eq!(ciphertext, expected_ciphertext_bytes);
    }
}